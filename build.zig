...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{
        .name = "soln.bin",
        .root_source_file = b.path("soln.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const exe2 = b.addExecutable(.{
        .name = "soln2.bin",
        .root_source_file = b.path("soln2.zig"),
        .target = target,
        .optimize = optimize,
    });
    const ul = b.addModule("ul", .{ .root_source_file = b.path("../../lib/zig/ul.zig") });
    exe.root_module.addImport("ul", ul);
    exe2.root_module.addImport("ul", ul);
